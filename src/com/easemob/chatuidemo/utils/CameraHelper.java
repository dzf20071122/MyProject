...
public class CameraHelper implements PreviewCallback {
    ...
    boolean isScreenOriatationPortrait() {
    	String tip="";
    	if(BMapApiApp.applicationContext==null){
    		tip+=" applicationContext=null";
    	}else if(BMapApiApp.applicationContext.getResources()==null){
    		tip+=" .getResources()";
    	}else if(BMapApiApp.applicationContext.getResources().getConfiguration()==null){
    		tip+=" .getConfiguration()";
    	}
    	
    	Log.i("Runt",String.format("CameraHelper tip:%s", tip));
        return BMapApiApp.applicationContext.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT;
    }
}
